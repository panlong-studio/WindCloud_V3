...
当前版本删除路径节点时**没有继续维护 `files.count` 和物理文件回收**，这是现架构下仍可继续完善的一点。
...
#### B. 秒传的判断依据是 `files.sha256sum`

如果 `dao_file_find_by_sha256()` 找到同哈希文件：
...
当前 `rm` / `rmdir` 主要删除 `paths` 逻辑节点。

对于普通文件，还可以继续补：

1. 删除路径节点后让 `files.count - 1`
2. 当 `count = 0` 时删除 `files` 记录
3. 再删除 `test/files/<sha256>` 物理文件
